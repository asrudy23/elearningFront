...
export function TeacherLayout({ children }: TeacherLayoutProps) {
  ...
  // --- EXPLICATION 2: Utiliser le hook pour obtenir la fonction de déconnexion et l'état de chargement ---
    ...
    <div className="min-h-screen bg-[#F8F9FA]">
      ...
      <div className={cn("fixed inset-0 z-50 lg:hidden", sidebarOpen ? "block" : "hidden")}>
        ...
        <div className="fixed left-0 top-0 h-full w-64 bg-white shadow-xl">
          ...
          <nav className="p-4 space-y-2">
            ...
            {/* --- EXPLICATION 3: Mettre à jour le bouton avec la nouvelle logique --- */}
      ...
      <div className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col">
        <div className="flex flex-col flex-grow bg-white border-r border-gray-200 epg-shadow">
          ...
          <div className="p-4 border-t">
            {/* --- EXPLICATION 3 (bis): Mettre à jour aussi le bouton desktop --- */}
            <Button variant="ghost" className="w-full justify-start text-red-600 hover:text-red-700 hover:bg-red-50" onClick={() => logout()} disabled={isPending}>
              <LogOut className="w-5 h-5 mr-3" />
              {isPending ? "Déconnexion..." : "Déconnexion"}
            </Button>
